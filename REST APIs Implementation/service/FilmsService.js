      ...
      db.all(sql1, [filmId], (err, rows) => {
          if (err)
            ...
            db.serialize(() => {
                ...
                const sql2 = "SELECT delegationId FROM reviews WHERE filmId = ?";
                db.all(sql2, [filmId], (err, rows) => {
                    if (err) {
                        db.run('ROLLBACK');
                        reject(err);
                    } else {
                        let ids = rows.map((row) => row.delegationId);
                        ...
                        db.run(sql3, [filmId], (err) => {
                            if (err) {
                                ...
                                const sql4 = 'DELETE FROM delegations WHERE id IN (' + ids.map(() => '?').join(',') + ')';
                                db.run(sql4, ids, (err) => {
                                    if (err) {
                                        db.run('ROLLBACK');
                                        reject(err);
